...
const Login = () => {
  ...
  const submitHandler = async (e) => {
    ...
    if (isLogin) {
      //login
      try {
        const res = await axios.post(`${USER_API_END_POINT}/login`, { email, password }, {    // { } yahi hai req.body me jo body tha
          headers: {
      ...
      //signup
      try {
        const res = await axios.post(`${USER_API_END_POINT}/register`, { name, username, email, password }, {  // { } yahi hai req.body me jo body tha
          headers: {
            "Content-Type": "application/json"
          },
          withCredentials: true
  ...
  return (
    <div className='w-screen h-screen flex items-center justify-center'>
      <div className='flex items-center justify-evenly w-[80%]'>
        <div>
          <img className='ml-0' width={"300px"} src="https://freepnglogo.com/images/all_img/1691832278twitter-x-logo-png.png" alt="twitter logo" />
        </div>
        <div>
          <div className='my-5'>
          ...
          <form onSubmit={submitHandler} className='flex flex-col w-[55%]' >
            {
              !isLogin && (<>
                ...
                <input type="text" onChange={(e) => setUsername(e.target.value)} value={username} placeholder='Username' className='outline-blue-500 border border-gray-800 px-3 py-2 rounded-full my-1 font-semibold' />
              </>)
            }
            ...
            <h1>{isLogin ? "Don't have an account?" : "Already have an account?"} <span onClick={loginSignupHandler} className='font-bold text-blue-600 cursor-pointe2 '>{isLogin ? "Signup" : "Login"}</span></h1>
          </form>
        </div>
      </div>
    </div>
